# GGUF VLM Batch Image Test — **SmolVLM2-256M-Video-Instruct Q8 + matching mmproj** (`smolvlm2-256m-video-q8-same_model_test`)

Service default: `ggml-org/SmolVLM2-256M-Video-Instruct-GGUF` (`SmolVLM2-256M-Video-Instruct-Q8_0.gguf` + matching mmproj).
...
```bash
cd ai-service
python tests/smolvlm2-256m-video-q8-same_model_test/test_image.py \
...
## Outputs (`tests/smolvlm2-256m-video-q8-same_model_test/results/<timestamp>/`)
...
Logs: `tests/smolvlm2-256m-video-q8-same_model_test/logs/`
...
Running the test from inside this folder with incorrect relative paths can generate junk paths such as `tests/smolvlm2-256m-video-q8-same_model_test/tests/...`. Always run from `ai-service` root as shown above; `.gitignore` excludes this accidental subtree.
